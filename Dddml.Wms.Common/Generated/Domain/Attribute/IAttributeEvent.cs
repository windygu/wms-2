// <autogenerated>
//   This file was generated by T4 code generator GenerateAttributeDomain.tt.
//   Any changes made to this file manually will be lost next time the file is regenerated.
// </autogenerated>

using System;
using System.Collections.Generic;
using Dddml.Wms.Specialization;
using Dddml.Wms.Domain;
using Dddml.Wms.Domain.Attribute;

namespace Dddml.Wms.Domain.Attribute
{
	public interface IAttributeEvent : IEvent, IEventDto, IGlobalIdentity<AttributeEventId>, ICreated<string>
	{
		AttributeEventId AttributeEventId { get; }

        bool ReadOnly { get; set; }

		string AttributeName { get; set; }

		string OrganizationId { get; set; }

		string Description { get; set; }

		bool? IsMandatory { get; set; }

		string AttributeValueType { get; set; }

		int? AttributeValueLength { get; set; }

		bool? IsList { get; set; }

		string FieldName { get; set; }

		string ReferenceId { get; set; }

		bool? Active { get; set; }

	}

	public interface IAttributeStateCreated : IAttributeEvent//, IAttributeStateProperties
	{
		IEnumerable<IAttributeValueStateCreated> AttributeValueEvents { get; }
		
		void AddAttributeValueEvent(IAttributeValueStateCreated e);

		IAttributeValueStateCreated NewAttributeValueStateCreated(string value);

		IEnumerable<IAttributeAliasStateCreated> AttributeAliasEvents { get; }
		
		void AddAttributeAliasEvent(IAttributeAliasStateCreated e);

		IAttributeAliasStateCreated NewAttributeAliasStateCreated(string code);

	
	}


	public interface IAttributeStateMergePatched : IAttributeEvent//, IAttributeStateProperties
	{
		bool IsPropertyAttributeNameRemoved { get; set; }

		bool IsPropertyOrganizationIdRemoved { get; set; }

		bool IsPropertyDescriptionRemoved { get; set; }

		bool IsPropertyIsMandatoryRemoved { get; set; }

		bool IsPropertyAttributeValueTypeRemoved { get; set; }

		bool IsPropertyAttributeValueLengthRemoved { get; set; }

		bool IsPropertyIsListRemoved { get; set; }

		bool IsPropertyFieldNameRemoved { get; set; }

		bool IsPropertyReferenceIdRemoved { get; set; }

		bool IsPropertyActiveRemoved { get; set; }

		IEnumerable<IAttributeValueEvent> AttributeValueEvents { get; }
		
		void AddAttributeValueEvent(IAttributeValueEvent e);

		IAttributeValueStateCreated NewAttributeValueStateCreated(string value);

		IAttributeValueStateMergePatched NewAttributeValueStateMergePatched(string value);

		IAttributeValueStateRemoved NewAttributeValueStateRemoved(string value);

		IEnumerable<IAttributeAliasEvent> AttributeAliasEvents { get; }
		
		void AddAttributeAliasEvent(IAttributeAliasEvent e);

		IAttributeAliasStateCreated NewAttributeAliasStateCreated(string code);

		IAttributeAliasStateMergePatched NewAttributeAliasStateMergePatched(string code);

		IAttributeAliasStateRemoved NewAttributeAliasStateRemoved(string code);


	}

	public interface IAttributeStateDeleted : IAttributeEvent
	{
		IEnumerable<IAttributeValueStateRemoved> AttributeValueEvents { get; }
		
		void AddAttributeValueEvent(IAttributeValueStateRemoved e);
		
		IAttributeValueStateRemoved NewAttributeValueStateRemoved(string value);

		IEnumerable<IAttributeAliasStateRemoved> AttributeAliasEvents { get; }
		
		void AddAttributeAliasEvent(IAttributeAliasStateRemoved e);
		
		IAttributeAliasStateRemoved NewAttributeAliasStateRemoved(string code);

	}


}

