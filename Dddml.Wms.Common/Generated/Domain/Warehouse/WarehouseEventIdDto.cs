// <autogenerated>
//   This file was generated by T4 code generator GenerateWarehouseDomain.tt.
//   Any changes made to this file manually will be lost next time the file is regenerated.
// </autogenerated>

using System;
using System.Collections.Generic;
using Dddml.Wms.Specialization;
using Dddml.Wms.Domain;
using Dddml.Wms.Domain.Warehouse;

namespace Dddml.Wms.Domain.Warehouse
{

	public class WarehouseEventIdDto
	{

		public WarehouseEventIdDto()
		{
		}

        public virtual WarehouseEventId ToWarehouseEventId()
        {
            WarehouseEventId v = new WarehouseEventId();
            v.WarehouseId = this.WarehouseId;
            v.Version = this.Version;
            return v;
        }

		public virtual string WarehouseId { 
			get;
			set;
		}

		public virtual long Version { 
			get;
			set;
		}


		public override bool Equals (object obj)
		{
			if (Object.ReferenceEquals (this, obj)) {
				return true;
			}

			WarehouseEventIdDto other = obj as WarehouseEventIdDto;
			if (other == null) {
				return false;
			}

			return true 
				&& Object.Equals (this.WarehouseId, other.WarehouseId)
				&& Object.Equals (this.Version, other.Version)
				;
		}

		public override int GetHashCode ()
		{
			int hash = 0;
			if (this.WarehouseId != null) {
				hash += 13 * this.WarehouseId.GetHashCode ();
			}
			if (this.Version != null) {
				hash += 13 * this.Version.GetHashCode ();
			}
			return hash;
		}

	}

}


