// <autogenerated>
//   This file was generated by T4 code generator GenerateGoodIdentificationMvoDomain.tt.
//   Any changes made to this file manually will be lost next time the file is regenerated.
// </autogenerated>

using System;
using System.Collections.Generic;
using Dddml.Wms.Specialization;
using Dddml.Wms.Domain;
using Dddml.Wms.Domain.GoodIdentificationMvo;
using Dddml.Wms.Domain.Product;

namespace Dddml.Wms.Domain.GoodIdentificationMvo
{

    public interface IGoodIdentificationMvoStateDto
    {
        ProductGoodIdentificationId ProductGoodIdentificationId
        {
            get;
            set;
        }

        string IdValue
        {
            get;
            set;
        }

        long? Version
        {
            get;
            set;
        }

        bool? Active
        {
            get;
            set;
        }

        string ProductProductTypeId
        {
            get;
            set;
        }

        string ProductPrimaryProductCategoryId
        {
            get;
            set;
        }

        string ProductManufacturerPartyId
        {
            get;
            set;
        }

        string ProductFacilityId
        {
            get;
            set;
        }

        DateTime? ProductIntroductionDate
        {
            get;
            set;
        }

        DateTime? ProductReleaseDate
        {
            get;
            set;
        }

        DateTime? ProductSupportDiscontinuationDate
        {
            get;
            set;
        }

        DateTime? ProductSalesDiscontinuationDate
        {
            get;
            set;
        }

        string ProductSalesDiscWhenNotAvail
        {
            get;
            set;
        }

        string ProductInternalName
        {
            get;
            set;
        }

        string ProductBrandName
        {
            get;
            set;
        }

        string ProductComments
        {
            get;
            set;
        }

        string ProductProductName
        {
            get;
            set;
        }

        string ProductDescription
        {
            get;
            set;
        }

        string ProductPriceDetailText
        {
            get;
            set;
        }

        string ProductSmallImageUrl
        {
            get;
            set;
        }

        string ProductMediumImageUrl
        {
            get;
            set;
        }

        string ProductLargeImageUrl
        {
            get;
            set;
        }

        string ProductDetailImageUrl
        {
            get;
            set;
        }

        string ProductOriginalImageUrl
        {
            get;
            set;
        }

        string ProductDetailScreen
        {
            get;
            set;
        }

        string ProductInventoryMessage
        {
            get;
            set;
        }

        string ProductInventoryItemTypeId
        {
            get;
            set;
        }

        string ProductRequireInventory
        {
            get;
            set;
        }

        string ProductQuantityUomId
        {
            get;
            set;
        }

        decimal? ProductQuantityIncluded
        {
            get;
            set;
        }

        long? ProductPiecesIncluded
        {
            get;
            set;
        }

        string ProductRequireAmount
        {
            get;
            set;
        }

        decimal? ProductFixedAmount
        {
            get;
            set;
        }

        string ProductAmountUomTypeId
        {
            get;
            set;
        }

        string ProductWeightUomId
        {
            get;
            set;
        }

        decimal? ProductShippingWeight
        {
            get;
            set;
        }

        decimal? ProductProductWeight
        {
            get;
            set;
        }

        string ProductHeightUomId
        {
            get;
            set;
        }

        decimal? ProductProductHeight
        {
            get;
            set;
        }

        decimal? ProductShippingHeight
        {
            get;
            set;
        }

        string ProductWidthUomId
        {
            get;
            set;
        }

        decimal? ProductProductWidth
        {
            get;
            set;
        }

        decimal? ProductShippingWidth
        {
            get;
            set;
        }

        string ProductDepthUomId
        {
            get;
            set;
        }

        decimal? ProductProductDepth
        {
            get;
            set;
        }

        decimal? ProductShippingDepth
        {
            get;
            set;
        }

        string ProductDiameterUomId
        {
            get;
            set;
        }

        decimal? ProductProductDiameter
        {
            get;
            set;
        }

        decimal? ProductProductRating
        {
            get;
            set;
        }

        string ProductRatingTypeEnum
        {
            get;
            set;
        }

        string ProductReturnable
        {
            get;
            set;
        }

        string ProductTaxable
        {
            get;
            set;
        }

        string ProductChargeShipping
        {
            get;
            set;
        }

        string ProductAutoCreateKeywords
        {
            get;
            set;
        }

        string ProductIncludeInPromotions
        {
            get;
            set;
        }

        string ProductIsVirtual
        {
            get;
            set;
        }

        string ProductIsVariant
        {
            get;
            set;
        }

        string ProductVirtualVariantMethodEnum
        {
            get;
            set;
        }

        string ProductInShippingBox
        {
            get;
            set;
        }

        string ProductDefaultShipmentBoxTypeId
        {
            get;
            set;
        }

        bool? ProductIsSerialNumbered
        {
            get;
            set;
        }

        bool? ProductIsManagedByLot
        {
            get;
            set;
        }

        string ProductAttributeSetId
        {
            get;
            set;
        }

        string ProductAttributeSetInstanceId
        {
            get;
            set;
        }

        string ProductCreatedBy
        {
            get;
            set;
        }

        DateTime? ProductCreatedAt
        {
            get;
            set;
        }

        string ProductUpdatedBy
        {
            get;
            set;
        }

        DateTime? ProductUpdatedAt
        {
            get;
            set;
        }

        bool? ProductActive
        {
            get;
            set;
        }

        long? ProductVersion
        {
            get;
            set;
        }

        string CreatedBy
        {
            get;
            set;
        }

        DateTime? CreatedAt
        {
            get;
            set;
        }

        string UpdatedBy
        {
            get;
            set;
        }

        DateTime? UpdatedAt
        {
            get;
            set;
        }

        IGoodIdentificationMvoState ToGoodIdentificationMvoState();

    }

}

