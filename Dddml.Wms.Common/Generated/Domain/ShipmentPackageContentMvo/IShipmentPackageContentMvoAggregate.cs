// <autogenerated>
//   This file was generated by T4 code generator GenerateShipmentPackageContentMvoDomain.tt.
//   Any changes made to this file manually will be lost next time the file is regenerated.
// </autogenerated>

using System;
using System.Collections.Generic;
using Dddml.Wms.Specialization;
using Dddml.Wms.Domain;
using Dddml.Wms.Domain.ShipmentPackageContentMvo;
using Dddml.Wms.Domain.ShipmentPackage;

namespace Dddml.Wms.Domain.ShipmentPackageContentMvo
{

	public partial interface IShipmentPackageContentMvoAggregate : IGlobalIdentity<ShipmentPackageContentId>
	{
		IShipmentPackageContentMvoState State { get; }

		IList<IEvent> Changes { get; }

		void ThrowOnInvalidStateTransition (ICommand c);

		void Create (ICreateShipmentPackageContentMvo c);

		void MergePatch (IMergePatchShipmentPackageContentMvo c);

		void Delete (IDeleteShipmentPackageContentMvo c);


	}

}

