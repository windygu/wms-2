    ...
    public DayPlanMvoStateDto[] getAll(@Context HttpServletRequest request,
        ...
        try {
            ...
            if (!StringHelper.isNullOrEmpty(filter)) {
                ...
                states = dayPlanMvoApplicationService.get(
                        DayPlanMvoResourceUtils.getQueryFilterMap(request.getParameterMap()),
                        DayPlanMvoResourceUtils.getQueryOrders(sort, getQueryOrderSeparator()),
                        firstResult, maxResults);
            }
    ...
    public long getCount(@Context HttpServletRequest request,
        ...
        try {
            ...
            if (!StringHelper.isNullOrEmpty(filter)) {
                ...
                count = dayPlanMvoApplicationService.getCount(DayPlanMvoResourceUtils.getQueryFilterMap(request.getParameterMap()));
            }
            return count;
...
*/
        ...
        public static DayPlanMvoStateDto[] toDayPlanMvoStateDtoArray(Iterable<DayPlanId> ids) {
            List<DayPlanMvoStateDto> states = new ArrayList<>();
            ids.forEach(id -> {
                DayPlanMvoStateDto dto = new DayPlanMvoStateDto();
                dto.setDayPlanId(new DayPlanIdDtoWrapper(id));
                states.add(dto);
            });
