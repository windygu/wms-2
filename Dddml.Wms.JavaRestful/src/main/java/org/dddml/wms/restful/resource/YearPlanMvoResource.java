package org.dddml.wms.restful.resource;

import java.util.*;
import javax.servlet.http.*;
import javax.validation.constraints.*;
import javax.ws.rs.*;
import javax.ws.rs.core.*;

import org.dddml.support.criterion.*;
import java.util.Date;
import org.dddml.wms.specialization.*;
import org.dddml.wms.domain.*;
import org.dddml.wms.domain.meta.*;

import com.alibaba.fastjson.*;
import org.apache.cxf.jaxrs.ext.PATCH;
import org.springframework.beans.factory.annotation.Autowired;
import org.dddml.support.criterion.TypeConverter;

@Path("YearPlanMvos")
@Produces(MediaType.APPLICATION_JSON)
public class YearPlanMvoResource {


    @Autowired
    private YearPlanMvoApplicationService yearPlanMvoApplicationService;


    @GET
    public YearPlanMvoStateDto[] getAll(@Context HttpServletRequest request,
                                   @QueryParam("sort") String sort,
                                   @QueryParam("fields") String fields,
                                   @QueryParam("firstResult") @DefaultValue("0") Integer firstResult,
                                   @QueryParam("maxResults") @DefaultValue("2147483647") Integer maxResults,
                                   @QueryParam("filter") String filter) {
        if (firstResult < 0) { firstResult = 0; }
        if (maxResults == null || maxResults < 1) { maxResults = Integer.MAX_VALUE; }
        try {

            Iterable<YearPlanMvoState> states = null; 
            if (!StringHelper.isNullOrEmpty(filter)) {
                states = yearPlanMvoApplicationService.get(
                        CriterionDto.toSubclass(
                                JSON.parseObject(filter, CriterionDto.class),
                                getCriterionTypeConverter(), getPropertyTypeResolver()),
                        YearPlanMvoResourceUtils.getQueryOrders(sort, getQueryOrderSeparator()),
                        firstResult, maxResults);
            } else {
                states = yearPlanMvoApplicationService.get(
                        YearPlanMvoResourceUtils.getQueryFilterMap(request.getParameterMap()),
                        YearPlanMvoResourceUtils.getQueryOrders(sort, getQueryOrderSeparator()),
                        firstResult, maxResults);
            }

            YearPlanMvoStateDto.DtoConverter dtoConverter = new YearPlanMvoStateDto.DtoConverter();
            if (StringHelper.isNullOrEmpty(fields)) {
                dtoConverter.setAllFieldsReturned(true);
            } else {
                dtoConverter.setReturnedFieldsString(fields);
            }
            return dtoConverter.toYearPlanMvoStateDtoArray(states);

        } catch (DomainError error) { throw error; } catch (Exception ex) { throw new DomainError("ExceptionCaught", ex); }
    }

    @GET
    @Path("{id}")
    public YearPlanMvoStateDto get(@PathParam("id") String id, @QueryParam("fields") String fields) {
        try {
            YearPlanId idObj = YearPlanMvoResourceUtils.parseIdString(id);
            YearPlanMvoState state = yearPlanMvoApplicationService.get(idObj);
            if (state == null) { return null; }

            YearPlanMvoStateDto.DtoConverter dtoConverter = new YearPlanMvoStateDto.DtoConverter();
            if (StringHelper.isNullOrEmpty(fields)) {
                dtoConverter.setAllFieldsReturned(true);
            } else {
                dtoConverter.setReturnedFieldsString(fields);
            }
            return dtoConverter.toYearPlanMvoStateDto(state);

        } catch (DomainError error) { throw error; } catch (Exception ex) { throw new DomainError("ExceptionCaught", ex); }
    }

    @Path("_count")
    @GET
    public long getCount(@Context HttpServletRequest request,
                         @QueryParam("filter") String filter) {
        try {
            long count = 0;
            if (!StringHelper.isNullOrEmpty(filter)) {
                count = yearPlanMvoApplicationService.getCount(CriterionDto.toSubclass(JSONObject.parseObject(filter, CriterionDto.class),
                        getCriterionTypeConverter(), getPropertyTypeResolver()));
            } else {
                count = yearPlanMvoApplicationService.getCount(YearPlanMvoResourceUtils.getQueryFilterMap(request.getParameterMap()));
            }
            return count;

        } catch (DomainError error) { throw error; } catch (Exception ex) { throw new DomainError("ExceptionCaught", ex); }
    }


    @PUT
    @Path("/{id}")
    public void put(@PathParam("id") String id, CreateOrMergePatchYearPlanMvoDto.CreateYearPlanMvoDto value) {
        try {

            YearPlanMvoCommand.CreateYearPlanMvo cmd = value.toCreateYearPlanMvo();
            YearPlanMvoResourceUtils.setNullIdOrThrowOnInconsistentIds(id, cmd);
            yearPlanMvoApplicationService.when(cmd);

        } catch (DomainError error) { throw error; } catch (Exception ex) { throw new DomainError("ExceptionCaught", ex); }
    }


    @PATCH
    @Path("/{id}")
    public void patch(@PathParam("id") String id, CreateOrMergePatchYearPlanMvoDto.MergePatchYearPlanMvoDto value) {
        try {

            YearPlanMvoCommand.MergePatchYearPlanMvo cmd = value.toMergePatchYearPlanMvo();
            YearPlanMvoResourceUtils.setNullIdOrThrowOnInconsistentIds(id, cmd);
            yearPlanMvoApplicationService.when(cmd);

        } catch (DomainError error) { throw error; } catch (Exception ex) { throw new DomainError("ExceptionCaught", ex); }
    }

    @DELETE
    @Path("/{id}")
    public void delete(@PathParam("id") String id,
                       @NotNull @QueryParam("commandId") String commandId,
                       @NotNull @QueryParam("version") @Min(value = -1) Long version,
                       @QueryParam("requesterId") String requesterId) {
        try {

            YearPlanMvoCommand.DeleteYearPlanMvo deleteCmd = new AbstractYearPlanMvoCommand.SimpleDeleteYearPlanMvo();

            deleteCmd.setCommandId(commandId);
            deleteCmd.setRequesterId(requesterId);
            deleteCmd.setPersonVersion(version);
            YearPlanMvoResourceUtils.setNullIdOrThrowOnInconsistentIds(id, deleteCmd);
            yearPlanMvoApplicationService.when(deleteCmd);

        } catch (DomainError error) { throw error; } catch (Exception ex) { throw new DomainError("ExceptionCaught", ex); }
    }

    @Path("_metadata/filteringFields")
    @GET
    public List<PropertyMetadataDto> getMetadataFilteringFields() {
        try {

            List<PropertyMetadataDto> filtering = new ArrayList<>();
            YearPlanMvoFilteringProperties.propertyTypeMap.forEach((key, value) -> {
                filtering.add(new PropertyMetadataDto(key, value, true));
            });
            return filtering;

        } catch (DomainError error) { throw error; } catch (Exception ex) { throw new DomainError("ExceptionCaught", ex); }
    }

    @Path("{id}/_stateEvents/{version}")
    @GET
    public YearPlanMvoStateEvent getStateEvent(@PathParam("id") String id, @PathParam("version") long version) {
        try {

            YearPlanId idObj = YearPlanMvoResourceUtils.parseIdString(id);
            return yearPlanMvoApplicationService.getStateEvent(idObj, version);

        } catch (DomainError error) { throw error; } catch (Exception ex) { throw new DomainError("ExceptionCaught", ex); }
    }


    protected String getQueryOrderSeparator() {
        return ",";
    }

    protected TypeConverter getCriterionTypeConverter() {
        return new DefaultTypeConverter();
    }

    protected PropertyTypeResolver getPropertyTypeResolver() {
        return new YearPlanMvoPropertyTypeResolver();
    }

    // ////////////////////////////////

    private class YearPlanMvoPropertyTypeResolver implements PropertyTypeResolver {
        @Override
        public Class resolveTypeByPropertyName(String propertyName) {
            return YearPlanMvoResourceUtils.getFilterPropertyType(propertyName);
        }
    }

 
    public static class YearPlanMvoResourceUtils {

        public static List<String> getQueryOrders(String str, String separator) {
            List<String> orders = new ArrayList<>();
            if (StringHelper.isNullOrEmpty(str)) {
                return orders;
            }
            String[] splits = str.split(separator);
            for (String item : splits) {
                if (!StringHelper.isNullOrEmpty(item)) {
                    orders.add(item);
                }
            }
            return orders;
        }

        public static void setNullIdOrThrowOnInconsistentIds(String id, YearPlanMvoCommand value) {
            YearPlanId idObj = parseIdString(id);
            if (value.getYearPlanId() == null) {
                value.setYearPlanId(idObj);
            } else if (!value.getYearPlanId().equals(idObj)) {
                throw DomainError.named("inconsistentId", "Argument Id %1$s NOT equals body Id %2$s", id, value.getYearPlanId());
            }
        }
    

        public static YearPlanId parseIdString(String idString) {
            YearPlanIdFlattenedDtoFormatter formatter = new YearPlanIdFlattenedDtoFormatter();
            YearPlanIdFlattenedDto idDto = formatter.parse(idString);
            return idDto.toYearPlanId();
        }


        public static String getFilterPropertyName(String fieldName) {
            if ("sort".equalsIgnoreCase(fieldName)
                    || "firstResult".equalsIgnoreCase(fieldName)
                    || "maxResults".equalsIgnoreCase(fieldName)
                    || "fields".equalsIgnoreCase(fieldName)) {
                return null;
            }
            if (YearPlanMvoFilteringProperties.propertyTypeMap.containsKey(fieldName)) {
/* TODO...
                var p = YearPlanMvoMetadata.Instance.PropertyMetadataDictionary[fieldName];
                if (p.IsFilteringProperty)
                {
                    var propertyName = fieldName;
                    if (p.IsDerived)
                    {
                        propertyName = p.DerivedFrom;
                    }
                    return propertyName;
                }
*/
                return fieldName;
            }
            return null;
        }

        public static Class getFilterPropertyType(String propertyName) {
            if (YearPlanMvoFilteringProperties.propertyTypeMap.containsKey(propertyName)) {
                String propertyType = YearPlanMvoFilteringProperties.propertyTypeMap.get(propertyName);
                if (!StringHelper.isNullOrEmpty(propertyType)) {
                    if (ReflectUtils.CLASS_MAP.containsKey(propertyType)) {
                        return ReflectUtils.CLASS_MAP.get(propertyType);
                    }
                }
            }
            return String.class;
        }

        public static Iterable<Map.Entry<String, Object>> getQueryFilterMap(Map<String, String[]> queryNameValuePairs) {
            Map<String, Object> filter = new HashMap<>();
            queryNameValuePairs.forEach((key, values) -> {
                if (values.length > 0) {
                    String pName = getFilterPropertyName(key);
                    if (!StringHelper.isNullOrEmpty(pName)) {
                        Class pClass = getFilterPropertyType(pName);
                        filter.put(pName, ApplicationContext.current.getTypeConverter().convertFromString(pClass, values[0]));
                    }
                }
            });
            return filter.entrySet();
        }

        public static YearPlanMvoStateDto[] toYearPlanMvoStateDtoArray(Iterable<YearPlanId> ids) {
            List<YearPlanMvoStateDto> states = new ArrayList<>();
            ids.forEach(id -> {
                YearPlanMvoStateDto dto = new YearPlanMvoStateDto();
                dto.setYearPlanId(new YearPlanIdDtoWrapper(id));
                states.add(dto);
            });
            return states.toArray(new YearPlanMvoStateDto[0]);
        }

    }

}

