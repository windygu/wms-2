...
import org.dddml.wms.domain.orderheader.OrderHeaderStateEvent.*;

public abstract class AbstractOrderHeaderState implements OrderHeaderState
{
    ...
    public AbstractOrderHeaderState(List<Event> events) {
        ...
        if (events != null && events.size() > 0) {
            this.setOrderId(((OrderHeaderStateEvent) events.get(0)).getStateEventId().getOrderId());
            for (Event e : events) {
                mutate(e);
    ...
    public AbstractOrderHeaderState(boolean forReapplying) {
        this.forReapplying = forReapplying;
    ...
    public void mutate(Event e) {
        ...
        if (e instanceof OrderHeaderStateCreated) {
            when((OrderHeaderStateCreated) e);
        } else if (e instanceof OrderHeaderStateMergePatched) {
            when((OrderHeaderStateMergePatched) e);
        } else {
            throw new UnsupportedOperationException(String.format("Unsupported event type: %1$s", e.getClass().getName()));
        }
    }

    public void when(OrderHeaderStateCreated e)
    {
        throwOnWrongEvent(e);
    ...
    public void when(OrderHeaderStateMergePatched e)
    {
        throwOnWrongEvent(e);
    ...
    protected void throwOnWrongEvent(OrderHeaderStateEvent stateEvent)
    {
        ...
        if (!(stateVersion == null && eventVersion.equals(OrderHeaderState.VERSION_NULL)) && !eventVersion.equals(stateVersion))
        {
    ...
    public static class SimpleOrderHeaderState extends AbstractOrderHeaderState
    {
        ...
        public SimpleOrderHeaderState(boolean forReapplying) {
            super(forReapplying);
        }
