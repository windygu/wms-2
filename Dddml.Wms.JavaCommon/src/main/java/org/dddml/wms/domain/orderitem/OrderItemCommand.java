...
public interface OrderItemCommand extends Command
{
    ...
    static void throwOnInvalidStateTransition(OrderItemState state, Command c) {
        if (state.getVersion() == null)
        {
            if (isCommandCreate((OrderItemCommand)c))
            {
                return;
            }
    ...
    static boolean isCommandCreate(OrderItemCommand c) {
        return ((c instanceof OrderItemCommand.CreateOrderItem) 
            && c.getVersion().equals(OrderItemState.VERSION_NULL));
    }
	...
	interface DeleteOrderItem extends OrderItemCommand
	{
	}
