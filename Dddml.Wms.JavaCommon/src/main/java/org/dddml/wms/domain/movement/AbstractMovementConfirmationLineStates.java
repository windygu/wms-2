...
public abstract class AbstractMovementConfirmationLineStates implements MovementConfirmationLineStates
{
    ...
    protected Iterable<MovementConfirmationLineState> getInnerIterable() {
        if (!getForReapplying()) {
            return getMovementConfirmationLineStateDao().findByMovementDocumentNumber(movementState.getDocumentNumber());
        } else {
    ...
    public AbstractMovementConfirmationLineStates(MovementState outerState) {
    ...
    public MovementConfirmationLineState get(String lineNumber, boolean forCreation, boolean nullAllowed) {
        MovementConfirmationLineId globalId = new MovementConfirmationLineId(movementState.getDocumentNumber(), lineNumber);
