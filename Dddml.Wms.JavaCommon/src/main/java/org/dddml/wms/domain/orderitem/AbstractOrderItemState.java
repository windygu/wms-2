...
import org.dddml.wms.domain.orderitem.OrderItemStateEvent.*;
...
public abstract class AbstractOrderItemState implements OrderItemState
{
    ...
    public AbstractOrderItemState(List<Event> events) {
        this(true);
        if (events != null && events.size() > 0) {
            this.setOrderItemId(((OrderItemStateEvent) events.get(0)).getStateEventId().getOrderItemId());
            for (Event e : events) {
                mutate(e);
    ...
    public static class SimpleOrderItemState extends AbstractOrderItemState
    {
        ...
        public SimpleOrderItemState(List<Event> events) {
            super(events);
        }
