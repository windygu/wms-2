...
public abstract class AbstractAttributeSetInstanceState implements AttributeSetInstanceState
{
    ...
    public AbstractAttributeSetInstanceState(List<Event> events) {
        ...
        if (events != null && events.size() > 0) {
            this.setAttributeSetInstanceId(((AttributeSetInstanceStateEvent) events.get(0)).getAttributeSetInstanceEventId().getAttributeSetInstanceId());
            for (Event e : events) {
                mutate(e);
    ...
    protected void throwOnWrongEvent(AttributeSetInstanceStateEvent stateEvent)
    {
        ...
        String eventEntityId = stateEvent.getAttributeSetInstanceEventId().getAttributeSetInstanceId(); // EntityBase.Aggregate.GetStateEventIdPropertyIdName();
        ...
        Long eventVersion = stateEvent.getAttributeSetInstanceEventId().getVersion();// Aggregate Version
        if (eventVersion == null) {
            throw new NullPointerException("stateEvent.getAttributeSetInstanceEventId().getVersion() == null");
        }
