...
public abstract class AbstractItemIssuanceMvoState implements ItemIssuanceMvoState
{
    ...
    public AbstractItemIssuanceMvoState() {
        this(false);
    }

    public AbstractItemIssuanceMvoState(boolean forReapplying) {
        this.forReapplying = forReapplying;
    ...
    public void mutate(Event e) {
        ...
        if (e instanceof ItemIssuanceMvoStateCreated) {
            when((ItemIssuanceMvoStateCreated) e);
        } else if (e instanceof ItemIssuanceMvoStateMergePatched) {
            when((ItemIssuanceMvoStateMergePatched) e);
        } else if (e instanceof ItemIssuanceMvoStateDeleted) {
            when((ItemIssuanceMvoStateDeleted) e);
        } else {
            throw new UnsupportedOperationException(String.format("Unsupported event type: %1$s", e.getClass().getName()));
        }
    }

    public void when(ItemIssuanceMvoStateCreated e)
    {
        throwOnWrongEvent(e);
    ...
    public void when(ItemIssuanceMvoStateMergePatched e)
    {
        throwOnWrongEvent(e);
    ...
    public void when(ItemIssuanceMvoStateDeleted e)
    {
        throwOnWrongEvent(e);
    ...
    protected void throwOnWrongEvent(ItemIssuanceMvoEvent event)
    {
        ShipmentItemIssuanceId stateEntityId = this.getShipmentItemIssuanceId(); // Aggregate Id
        ShipmentItemIssuanceId eventEntityId = event.getItemIssuanceMvoEventId().getShipmentItemIssuanceId(); // EntityBase.Aggregate.GetEventIdPropertyIdName();
        ...
        Long eventVersion = event.getItemIssuanceMvoEventId().getShipmentVersion();// Aggregate Version
        if (eventVersion == null) {
            throw new NullPointerException("event.getItemIssuanceMvoEventId().getShipmentVersion() == null");
        }
        if (!(stateVersion == null && eventVersion.equals(ItemIssuanceMvoState.VERSION_NULL)) && !eventVersion.equals(stateVersion))
        {
    ...
    public static class SimpleItemIssuanceMvoState extends AbstractItemIssuanceMvoState
    {

        public SimpleItemIssuanceMvoState() {
        }

        public SimpleItemIssuanceMvoState(boolean forReapplying) {
            super(forReapplying);
        }

        public SimpleItemIssuanceMvoState(List<Event> events) {
            super(events);
        }
