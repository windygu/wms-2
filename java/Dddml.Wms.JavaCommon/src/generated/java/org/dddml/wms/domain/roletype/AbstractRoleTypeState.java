...
public abstract class AbstractRoleTypeState implements RoleTypeState
{
    ...
    protected void throwOnWrongEvent(RoleTypeStateEvent stateEvent)
    {
        ...
        Long eventVersion = stateEvent.getRoleTypeEventId().getVersion();// Aggregate Version
        if (eventVersion == null) {
            throw new NullPointerException("stateEvent.getRoleTypeEventId().getVersion() == null");
        }
        if (!(stateVersion == null && eventVersion.equals(RoleTypeState.VERSION_NULL)) && !eventVersion.equals(stateVersion))//(eventVersion.compareTo(stateVersion) >= 0)
        {
