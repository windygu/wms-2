...
namespace Dddml.Wms.HttpServices.ClientProxies
{
    ...
    public partial class RoleApplicationServiceProxy : IRoleApplicationService
    {
        ...
        public async Task<IRoleState> GetAsync(string roleId)
        {
            ...
            RoleProxyUtils.ThrowOnHttpResponseError(resp);
            state = (resp.Content == null) ? null : resp.Content.ToRoleState();
            return state;
        }
        ...
        public async Task<IEnumerable<IRoleState>> GetAsync(IEnumerable<KeyValuePair<string, object>> filter, IList<string> orders = null, int firstResult = 0, int maxResults = int.MaxValue, IList<string> fields = null)
        {
            ...
            RoleProxyUtils.ThrowOnHttpResponseError(resp);
            states = (resp.Content == null) ? null : resp.Content.Select(e => e.ToRoleState());
            return states;
        }
        ...
        public async Task<IRoleState> GetHistoryStateAsync(string roleId, long version)
        {
            var idObj = roleId;
            var uriParameters = new RoleHistoryStateUriParameters();
            uriParameters.Id = idObj;
            uriParameters.Version = version.ToString();

            var req = new RoleHistoryStateGetRequest(uriParameters);
            var resp = await _ramlClient.RoleHistoryState.Get(req);
            RoleProxyUtils.ThrowOnHttpResponseError(resp);
            return (resp.Content == null) ? null : resp.Content.ToRoleState();
        }

        public virtual IRoleState GetHistoryState(string roleId, long version)
        {
            return GetHistoryStateAsync(roleId, version).GetAwaiter().GetResult();
        }
