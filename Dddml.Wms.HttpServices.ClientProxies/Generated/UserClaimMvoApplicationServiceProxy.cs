...
namespace Dddml.Wms.HttpServices.ClientProxies
{
    ...
    public partial class UserClaimMvoApplicationServiceProxy : IUserClaimMvoApplicationService
    {
        ...
        public async Task<IUserClaimMvoState> GetAsync(UserClaimId userClaimId)
        {
            ...
            UserClaimMvoProxyUtils.ThrowOnHttpResponseError(resp);
            state = (resp.Content == null) ? null : resp.Content.ToUserClaimMvoState();
            return state;
        }
        ...
        public async Task<IEnumerable<IUserClaimMvoState>> GetAsync(IEnumerable<KeyValuePair<string, object>> filter, IList<string> orders = null, int firstResult = 0, int maxResults = int.MaxValue, IList<string> fields = null)
        {
            ...
            UserClaimMvoProxyUtils.ThrowOnHttpResponseError(resp);
            states = (resp.Content == null) ? null : resp.Content.Select(e => e.ToUserClaimMvoState());
            return states;
        }
        ...
        public async Task<IUserClaimMvoState> GetHistoryStateAsync(UserClaimId userClaimId, long version)
        {
            var idObj = UserClaimMvoProxyUtils.ToIdString(userClaimId);
            var uriParameters = new UserClaimMvoHistoryStateUriParameters();
            uriParameters.Id = idObj;
            uriParameters.Version = version.ToString();

            var req = new UserClaimMvoHistoryStateGetRequest(uriParameters);
            var resp = await _ramlClient.UserClaimMvoHistoryState.Get(req);
            UserClaimMvoProxyUtils.ThrowOnHttpResponseError(resp);
            return (resp.Content == null) ? null : resp.Content.ToUserClaimMvoState();
        }

        public virtual IUserClaimMvoState GetHistoryState(UserClaimId userClaimId, long version)
        {
            return GetHistoryStateAsync(userClaimId, version).GetAwaiter().GetResult();
        }
