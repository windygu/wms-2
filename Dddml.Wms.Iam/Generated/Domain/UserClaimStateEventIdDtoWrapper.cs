// <autogenerated>
//   This file was generated by T4 code generator GenerateBoundedContextDomainAggregates.tt.
//   Any changes made to this file manually will be lost next time the file is regenerated.
// </autogenerated>

using System;
using System.Collections.Generic;
using Dddml.Wms.Specialization;
using Dddml.Wms.Domain;
using Dddml.Wms.Domain.User;

namespace Dddml.Wms.Domain.User
{

	public class UserClaimStateEventIdDtoWrapper : UserClaimStateEventIdDto
	{

        private UserClaimStateEventId _value = new UserClaimStateEventId();

		public UserClaimStateEventIdDtoWrapper()
		{
		}

		public UserClaimStateEventIdDtoWrapper(UserClaimStateEventId val)
		{
			if (val == null) { throw new ArgumentNullException("val"); }
			this._value = val;
		}

        public override UserClaimStateEventId ToUserClaimStateEventId()
        {
            return this._value;
        }

		public override string UserId { 
			get { return _value.UserId; } 
			set { _value.UserId = value; } 
		}

		public override int ClaimId { 
			get { return _value.ClaimId; } 
			set { _value.ClaimId = value; } 
		}

		public override long UserVersion { 
			get { return _value.UserVersion; } 
			set { _value.UserVersion = value; } 
		}


	}

}


