...
namespace Dddml.Wms.HttpServices.ApiControllers
{
    ...
    public partial class AttributeSetsController : ApiController
    {
        ...
        public IEnumerable<IAttributeSetStateDto> GetAll(string sort = null, string fields = null, int firstResult = 0, int maxResults = int.MaxValue, string filter = null)
        {
          try {
            ...
            {
                states = _attributeSetApplicationService.Get(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(), new WebApiControllerTypeConverter(), new PropertyTypeResolver()
                    , n => (AttributeSetMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(n) ? AttributeSetMetadata.Instance.FilteringPropertyAliasDictionary[n] : n))
            ...
            {
                var dto = s is AttributeSetStateDtoWrapper ? (AttributeSetStateDtoWrapper)s : new AttributeSetStateDtoWrapper(s);
        ...
        public IAttributeSetStateDto Get(string id, string fields = null)
        {
          try {
            var idObj = id;
            var state = _attributeSetApplicationService.Get(idObj);
        ...
        public long GetCount(string filter = null)
        {
          try {
            ...
            if (!String.IsNullOrWhiteSpace(filter))
            {
                count = _attributeSetApplicationService.GetCount(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(), new WebApiControllerTypeConverter(), new PropertyTypeResolver()
                    , n => (AttributeSetMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(n) ? AttributeSetMetadata.Instance.FilteringPropertyAliasDictionary[n] : n)));
            }
            else 
            {
        ...
        [HttpGet]
        public AttributeSetStateCreatedOrMergePatchedOrDeletedDto GetStateEvent(string id, long version)
        {
          try {
            var idObj = id;
            var conv = new AttributeSetStateEventDtoConverter();
            var se = _attributeSetApplicationService.GetEvent(idObj, version);
        ...
        public IAttributeSetStateDto GetHistoryState(string id, long version, string fields = null)
        {
          try {
            var idObj = id;
            var state = _attributeSetApplicationService.GetHistoryState(idObj, version);
            if (state == null) { return null; }
            var stateDto = new AttributeSetStateDtoWrapper(state);
            if (String.IsNullOrWhiteSpace(fields))
            {
    ...
    public static class AttributeSetsControllerUtils
    {
        ...
        public static string GetFilterPropertyName(string fieldName)
        {
            ...
            if (AttributeSetMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(fieldName))
            {
                return AttributeSetMetadata.Instance.FilteringPropertyAliasDictionary[fieldName];
            }
            return null;
        }
