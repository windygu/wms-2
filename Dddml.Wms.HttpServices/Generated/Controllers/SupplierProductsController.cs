...
namespace Dddml.Wms.HttpServices.ApiControllers
{
    ...
    [RoutePrefix("api/SupplierProducts")]
    public partial class SupplierProductsController : ApiController
    {

        ISupplierProductApplicationService _supplierProductApplicationService = ApplicationContext.Current["SupplierProductApplicationService"] as ISupplierProductApplicationService;

        [Route(Order = 1)]
        [HttpGet]
        public IEnumerable<ISupplierProductStateDto> GetAll(string sort = null, string fields = null, int firstResult = 0, int maxResults = int.MaxValue, string filter = null)
        {
          try {
            IEnumerable<ISupplierProductState> states = null; 
            if (!String.IsNullOrWhiteSpace(filter))
            {
                states = _supplierProductApplicationService.Get(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(), new ApiControllerTypeConverter(), new PropertyTypeResolver()
                    , n => (SupplierProductMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(n) ? SupplierProductMetadata.Instance.FilteringPropertyAliasDictionary[n] : n))
                    , SupplierProductsControllerUtils.GetQueryOrders(sort, QueryOrderSeparator), firstResult, maxResults);
            }
            else 
            {
                states = _supplierProductApplicationService.Get(SupplierProductsControllerUtils.GetQueryFilterDictionary(this.Request.GetQueryNameValuePairs())
                    , SupplierProductsControllerUtils.GetQueryOrders(sort, QueryOrderSeparator), firstResult, maxResults);
            }
            var stateDtos = new List<ISupplierProductStateDto>();
            foreach (var s in states)
            {
                var dto = s is SupplierProductStateDtoWrapper ? (SupplierProductStateDtoWrapper)s : new SupplierProductStateDtoWrapper(s);
        ...
        [HttpGet]
        public ISupplierProductStateDto Get(string id, string fields = null)
        {
          try {
            var idObj = SupplierProductsControllerUtils.ParseIdString(id);
            var state = _supplierProductApplicationService.Get(idObj);
            if (state == null) { return null; }
            var stateDto = new SupplierProductStateDtoWrapper(state);
            if (String.IsNullOrWhiteSpace(fields))
            {
        ...
        public long GetCount(string filter = null)
        {
          try
          {
            ...
            if (!String.IsNullOrWhiteSpace(filter))
            {
                count = _supplierProductApplicationService.GetCount(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(), new ApiControllerTypeConverter(), new PropertyTypeResolver()
                    , n => (SupplierProductMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(n) ? SupplierProductMetadata.Instance.FilteringPropertyAliasDictionary[n] : n)));
            }
            else 
            {
                count = _supplierProductApplicationService.GetCount(SupplierProductsControllerUtils.GetQueryFilterDictionary(this.Request.GetQueryNameValuePairs()));
            }
            return count;
          } catch (Exception ex) { var response = SupplierProductsControllerUtils.GetErrorHttpResponseMessage(ex); throw new HttpResponseException(response); }
        }
        ...
        public void Put(string id, [FromBody]CreateSupplierProductDto value)
        {
          try {
        ...
        public IEnumerable<PropertyMetadataDto> GetMetadataFilteringFields()
        {
          try {
            ...
            foreach (var p in SupplierProductMetadata.Instance.Properties)
            {
        ...
        [HttpGet]
        public SupplierProductStateCreatedOrMergePatchedOrDeletedDto GetStateEvent(string id, long version)
        {
          try {
            var idObj = SupplierProductsControllerUtils.ParseIdString(id);
            var conv = new SupplierProductStateEventDtoConverter();
            var se = _supplierProductApplicationService.GetStateEvent(idObj, version);
            return se == null ? null : conv.ToSupplierProductStateEventDto(se);
          } catch (Exception ex) { var response = SupplierProductsControllerUtils.GetErrorHttpResponseMessage(ex); throw new HttpResponseException(response); }
        }

        [Route("{id}/_historyStates/{version}")]
        [HttpGet]
        public ISupplierProductStateDto GetHistoryState(string id, long version, string fields = null)
        {
          try {
            var idObj = SupplierProductsControllerUtils.ParseIdString(id);
            var state = _supplierProductApplicationService.GetHistoryState(idObj, version);
            if (state == null) { return null; }
            var stateDto = new SupplierProductStateDtoWrapper(state);
            if (String.IsNullOrWhiteSpace(fields))
            {
        ...
        private class PropertyTypeResolver : IPropertyTypeResolver
        {
            ...
            {
                return SupplierProductsControllerUtils.GetFilterPropertyType(propertyName);
            }
        }
    ...
    public static class SupplierProductsControllerUtils
    {
        ...
        public static string GetFilterPropertyName(string fieldName)
        {
            ...
            if (SupplierProductMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(fieldName))
            {
                return SupplierProductMetadata.Instance.FilteringPropertyAliasDictionary[fieldName];
            }
            return null;
        }
        ...
        {
            if (SupplierProductMetadata.Instance.PropertyMetadataDictionary.ContainsKey(propertyName))
            {
                return SupplierProductMetadata.Instance.PropertyMetadataDictionary[propertyName].Type;
            }
        ...
        public static IEnumerable<ISupplierProductStateDto> ToSupplierProductStateDtoCollection(IEnumerable<SupplierProductId> ids)
        {
            var states = new List<ISupplierProductStateDto>();
