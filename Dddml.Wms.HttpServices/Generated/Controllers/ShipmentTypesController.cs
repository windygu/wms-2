...
namespace Dddml.Wms.HttpServices.ApiControllers
{
    ...
    [RoutePrefix("api/ShipmentTypes")]
    public partial class ShipmentTypesController : ApiController
    {

        IShipmentTypeApplicationService _shipmentTypeApplicationService = ApplicationContext.Current["ShipmentTypeApplicationService"] as IShipmentTypeApplicationService;

        [Route(Order = 1)]
        [HttpGet]
        public IEnumerable<IShipmentTypeStateDto> GetAll(string sort = null, string fields = null, int firstResult = 0, int maxResults = int.MaxValue, string filter = null)
        {
          try {
            IEnumerable<IShipmentTypeState> states = null; 
            if (!String.IsNullOrWhiteSpace(filter))
            {
                states = _shipmentTypeApplicationService.Get(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(), new WebApiControllerTypeConverter(), new PropertyTypeResolver()
                    , n => (ShipmentTypeMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(n) ? ShipmentTypeMetadata.Instance.FilteringPropertyAliasDictionary[n] : n))
                    , ShipmentTypesControllerUtils.GetQueryOrders(sort, QueryOrderSeparator), firstResult, maxResults);
            }
            else 
            {
                states = _shipmentTypeApplicationService.Get(ShipmentTypesControllerUtils.GetQueryFilterDictionary(this.Request.GetQueryNameValuePairs())
                    , ShipmentTypesControllerUtils.GetQueryOrders(sort, QueryOrderSeparator), firstResult, maxResults);
            }
            var stateDtos = new List<IShipmentTypeStateDto>();
            foreach (var s in states)
            {
                var dto = s is ShipmentTypeStateDtoWrapper ? (ShipmentTypeStateDtoWrapper)s : new ShipmentTypeStateDtoWrapper(s);
        ...
        [HttpGet]
        public IShipmentTypeStateDto Get(string id, string fields = null)
        {
          try {
            var idObj = id;
            var state = _shipmentTypeApplicationService.Get(idObj);
            if (state == null) { return null; }
            var stateDto = new ShipmentTypeStateDtoWrapper(state);
            if (String.IsNullOrWhiteSpace(fields))
            {
        ...
        public long GetCount(string filter = null)
        {
          try {
            ...
            if (!String.IsNullOrWhiteSpace(filter))
            {
                count = _shipmentTypeApplicationService.GetCount(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(), new WebApiControllerTypeConverter(), new PropertyTypeResolver()
                    , n => (ShipmentTypeMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(n) ? ShipmentTypeMetadata.Instance.FilteringPropertyAliasDictionary[n] : n)));
            }
            else 
            {
                count = _shipmentTypeApplicationService.GetCount(ShipmentTypesControllerUtils.GetQueryFilterDictionary(this.Request.GetQueryNameValuePairs()));
            }
            return count;
          } catch (Exception ex) { var response = HttpServiceExceptionUtils.GetErrorHttpResponseMessage(ex); throw new HttpResponseException(response); }
        }
        ...
        public IEnumerable<PropertyMetadataDto> GetMetadataFilteringFields()
        {
          try {
            ...
            foreach (var p in ShipmentTypeMetadata.Instance.Properties)
            {
        ...
        private class PropertyTypeResolver : IPropertyTypeResolver
        {
            ...
            {
                return ShipmentTypesControllerUtils.GetFilterPropertyType(propertyName);
            }
        }
    ...
    public static class ShipmentTypesControllerUtils
    {
        ...
        public static string GetFilterPropertyName(string fieldName)
        {
            ...
            if (ShipmentTypeMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(fieldName))
            {
                return ShipmentTypeMetadata.Instance.FilteringPropertyAliasDictionary[fieldName];
            }
            return null;
        }
        ...
        {
            if (ShipmentTypeMetadata.Instance.PropertyMetadataDictionary.ContainsKey(propertyName))
            {
                return ShipmentTypeMetadata.Instance.PropertyMetadataDictionary[propertyName].Type;
            }
        ...
        public static IEnumerable<IShipmentTypeStateDto> ToShipmentTypeStateDtoCollection(IEnumerable<string> ids)
        {
            var states = new List<IShipmentTypeStateDto>();
            foreach (var id in ids)
            {
                var dto = new ShipmentTypeStateDtoWrapper();
                dto.ShipmentTypeId = id;
                states.Add(dto);
            }
            return states;
        }
