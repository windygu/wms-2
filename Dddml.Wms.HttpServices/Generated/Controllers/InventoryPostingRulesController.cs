...
namespace Dddml.Wms.HttpServices.ApiControllers
{
    ...
    [RoutePrefix("api/InventoryPostingRules")]
    public partial class InventoryPostingRulesController : ApiController
    {

        IInventoryPostingRuleApplicationService _inventoryPostingRuleApplicationService = ApplicationContext.Current["InventoryPostingRuleApplicationService"] as IInventoryPostingRuleApplicationService;
        ...
        public long GetCount(string filter = null)
        {
          try
          {
            ...
            if (!String.IsNullOrWhiteSpace(filter))
            {
                count = _inventoryPostingRuleApplicationService.GetCount(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(), new ApiControllerTypeConverter(), new PropertyTypeResolver()
                    , n => (InventoryPostingRuleMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(n) ? InventoryPostingRuleMetadata.Instance.FilteringPropertyAliasDictionary[n] : n)));
            }
            else 
            {
        ...
        [HttpGet]
        public IInventoryPostingRuleStateDto GetHistoryState(string id, long version, string fields = null)
        {
          try {
            var idObj = id;
            var state = _inventoryPostingRuleApplicationService.GetHistoryState(idObj, version);
            if (state == null) { return null; }
            var stateDto = new InventoryPostingRuleStateDtoWrapper(state);
            if (String.IsNullOrWhiteSpace(fields))
            {
    ...
    public static class InventoryPostingRulesControllerUtils
    {
        ...
        public static string GetFilterPropertyName(string fieldName)
        {
            ...
            if (InventoryPostingRuleMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(fieldName))
            {
                return InventoryPostingRuleMetadata.Instance.FilteringPropertyAliasDictionary[fieldName];
            }
            return null;
        }
        ...
        {
            if (InventoryPostingRuleMetadata.Instance.PropertyMetadataDictionary.ContainsKey(propertyName))
            {
                return InventoryPostingRuleMetadata.Instance.PropertyMetadataDictionary[propertyName].Type;
            }
        ...
        public static IEnumerable<IInventoryPostingRuleStateDto> ToInventoryPostingRuleStateDtoCollection(IEnumerable<string> ids)
        {
            var states = new List<IInventoryPostingRuleStateDto>();
            foreach (var id in ids)
            {
                var dto = new InventoryPostingRuleStateDtoWrapper();
                dto.InventoryPostingRuleId = id;
                states.Add(dto);
            }
            return states;
        }
