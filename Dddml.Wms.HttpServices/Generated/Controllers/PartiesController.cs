...
namespace Dddml.Wms.HttpServices.ApiControllers
{
    ...
    public partial class PartiesController : ApiController
    {
        ...
        public IEnumerable<IPartyStateDto> GetAll(string sort = null, string fields = null, int firstResult = 0, int maxResults = int.MaxValue, string filter = null)
        {
          try {
            ...
            {
                states = _partyApplicationService.Get(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(), new WebApiControllerTypeConverter(), new PropertyTypeResolver()
            ...
            {
                states = _partyApplicationService.Get(PartiesControllerUtils.GetQueryFilterDictionary(this.Request.GetQueryNameValuePairs())
                    , PartiesControllerUtils.GetQueryOrders(sort, QueryOrderSeparator), firstResult, maxResults);
            }
        ...
        public long GetCount(string filter = null)
        {
          try {
            ...
            if (!String.IsNullOrWhiteSpace(filter))
            {
                count = _partyApplicationService.GetCount(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(), new WebApiControllerTypeConverter(), new PropertyTypeResolver()
                    , n => (PartyMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(n) ? PartyMetadata.Instance.FilteringPropertyAliasDictionary[n] : n)));
            }
            else 
            {
                count = _partyApplicationService.GetCount(PartiesControllerUtils.GetQueryFilterDictionary(this.Request.GetQueryNameValuePairs()));
            }
            return count;
          } catch (Exception ex) { var response = HttpServiceExceptionUtils.GetErrorHttpResponseMessage(ex); throw new HttpResponseException(response); }
        }
    ...
    public static class PartiesControllerUtils
    {
