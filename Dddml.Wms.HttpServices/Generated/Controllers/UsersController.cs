    ...
    public partial class UsersController : ApiController
    {
        ...
        public IEnumerable<IUserStateDto> GetAll(string sort = null, string fields = null, int firstResult = 0, int maxResults = int.MaxValue, string filter = null)
        {
          try {
            ...
            {
                var dto = s is UserStateDtoWrapper ? (UserStateDtoWrapper)s : new UserStateDtoWrapper(s);
        ...
        public IUserStateDto Get(string id, string fields = null)
        {
          try {
            var idObj = id;
            var state = _userApplicationService.Get(idObj);
        ...
        public UserStateCreatedOrMergePatchedOrDeletedDto GetStateEvent(string id, long version)
        {
          try {
            ...
            var se = _userApplicationService.GetStateEvent(idObj, version);
            return se == null ? null : conv.ToUserStateEventDto(se);
          } catch (Exception ex) { var response = UsersControllerUtils.GetErrorHttpResponseMessage(ex); throw new HttpResponseException(response); }
        }
        ...
        public IUserStateDto GetHistoryState(string id, long version, string fields = null)
        {
          try {
            var idObj = id;
            var state = _userApplicationService.GetHistoryState(idObj, version);
            if (state == null) { return null; }
            var stateDto = new UserStateDtoWrapper(state);
            if (String.IsNullOrWhiteSpace(fields))
            {
