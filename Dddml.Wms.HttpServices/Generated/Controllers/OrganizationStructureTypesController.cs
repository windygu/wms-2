...
namespace Dddml.Wms.HttpServices.ApiControllers
{
    ...
    public partial class OrganizationStructureTypesController : ApiController
    {
        ...
        public IEnumerable<IOrganizationStructureTypeStateDto> GetAll(string sort = null, string fields = null, int firstResult = 0, int maxResults = int.MaxValue, string filter = null)
        {
          try {
            ...
            {
                states = _organizationStructureTypeApplicationService.Get(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(), new WebApiControllerTypeConverter(), new PropertyTypeResolver()
                    , n => (OrganizationStructureTypeMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(n) ? OrganizationStructureTypeMetadata.Instance.FilteringPropertyAliasDictionary[n] : n))
        ...
        public long GetCount(string filter = null)
        {
          try {
            ...
            if (!String.IsNullOrWhiteSpace(filter))
            {
                count = _organizationStructureTypeApplicationService.GetCount(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(), new WebApiControllerTypeConverter(), new PropertyTypeResolver()
                    , n => (OrganizationStructureTypeMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(n) ? OrganizationStructureTypeMetadata.Instance.FilteringPropertyAliasDictionary[n] : n)));
            }
            else 
            {
        ...
        public OrganizationStructureTypeStateCreatedOrMergePatchedOrDeletedDto GetStateEvent(string id, long version)
        {
          try {
            ...
            var se = _organizationStructureTypeApplicationService.GetEvent(idObj, version);
        ...
        public IOrganizationStructureTypeStateDto GetHistoryState(string id, long version, string fields = null)
        {
          try {
            var idObj = id;
            var state = _organizationStructureTypeApplicationService.GetHistoryState(idObj, version);
            if (state == null) { return null; }
            var stateDto = new OrganizationStructureTypeStateDtoWrapper(state);
            if (String.IsNullOrWhiteSpace(fields))
            {
    ...
    public static class OrganizationStructureTypesControllerUtils
    {
        ...
        public static string GetFilterPropertyName(string fieldName)
        {
            ...
            if (OrganizationStructureTypeMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(fieldName))
            {
                return OrganizationStructureTypeMetadata.Instance.FilteringPropertyAliasDictionary[fieldName];
            }
            return null;
        }
