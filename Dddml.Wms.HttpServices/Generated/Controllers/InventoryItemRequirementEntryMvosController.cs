...
namespace Dddml.Wms.HttpServices.ApiControllers
{
    ...
    [RoutePrefix("api/InventoryItemRequirementEntryMvos")]
    public partial class InventoryItemRequirementEntryMvosController : ApiController
    {

        IInventoryItemRequirementEntryMvoApplicationService _inventoryItemRequirementEntryMvoApplicationService = ApplicationContext.Current["InventoryItemRequirementEntryMvoApplicationService"] as IInventoryItemRequirementEntryMvoApplicationService;

        [Route(Order = 1)]
        [HttpGet]
        public IEnumerable<IInventoryItemRequirementEntryMvoStateDto> GetAll(string sort = null, string fields = null, int firstResult = 0, int maxResults = int.MaxValue, string filter = null)
        {
          try {
            IEnumerable<IInventoryItemRequirementEntryMvoState> states = null; 
            if (!String.IsNullOrWhiteSpace(filter))
            {
                states = _inventoryItemRequirementEntryMvoApplicationService.Get(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(),new ApiControllerTypeConverter(), new PropertyTypeResolver()
                    , n => (InventoryItemRequirementEntryMvoMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(n) ? InventoryItemRequirementEntryMvoMetadata.Instance.FilteringPropertyAliasDictionary[n] : n))
                    , InventoryItemRequirementEntryMvosControllerUtils.GetQueryOrders(sort, QueryOrderSeparator), firstResult, maxResults);
            }
            else 
            {
                states = _inventoryItemRequirementEntryMvoApplicationService.Get(InventoryItemRequirementEntryMvosControllerUtils.GetQueryFilterDictionary(this.Request.GetQueryNameValuePairs())
                    , InventoryItemRequirementEntryMvosControllerUtils.GetQueryOrders(sort, QueryOrderSeparator), firstResult, maxResults);
            }
            var stateDtos = new List<IInventoryItemRequirementEntryMvoStateDto>();
            foreach (var s in states)
            {
                var dto = s is InventoryItemRequirementEntryMvoStateDtoWrapper ? (InventoryItemRequirementEntryMvoStateDtoWrapper)s : new InventoryItemRequirementEntryMvoStateDtoWrapper(s);
        ...
        [HttpGet]
        public IInventoryItemRequirementEntryMvoStateDto Get(string id, string fields = null)
        {
          try {
            var idObj = InventoryItemRequirementEntryMvosControllerUtils.ParseIdString(id);
            var state = _inventoryItemRequirementEntryMvoApplicationService.Get(idObj);
            if (state == null) { return null; }
            var stateDto = new InventoryItemRequirementEntryMvoStateDtoWrapper(state);
            if (String.IsNullOrWhiteSpace(fields))
            {
        ...
        public long GetCount(string filter = null)
        {
          try
          {
            ...
            if (!String.IsNullOrWhiteSpace(filter))
            {
                count = _inventoryItemRequirementEntryMvoApplicationService.GetCount(CriterionDto.ToSubclass(JObject.Parse(filter).ToObject<CriterionDto>(),new ApiControllerTypeConverter(), new PropertyTypeResolver()
                    , n => (InventoryItemRequirementEntryMvoMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(n) ? InventoryItemRequirementEntryMvoMetadata.Instance.FilteringPropertyAliasDictionary[n] : n)));
            }
            else 
            {
                count = _inventoryItemRequirementEntryMvoApplicationService.GetCount(InventoryItemRequirementEntryMvosControllerUtils.GetQueryFilterDictionary(this.Request.GetQueryNameValuePairs()));
            }
            return count;
          } catch (Exception ex) { var response = InventoryItemRequirementEntryMvosControllerUtils.GetErrorHttpResponseMessage(ex); throw new HttpResponseException(response); }
        }
        ...
        public void Patch(string id, [FromBody]MergePatchInventoryItemRequirementEntryMvoDto value)
        {
          try {
            InventoryItemRequirementEntryMvosControllerUtils.SetNullIdOrThrowOnInconsistentIds(id, value);
        ...
        public IEnumerable<PropertyMetadataDto> GetMetadataFilteringFields()
        {
          try {
            ...
            foreach (var p in InventoryItemRequirementEntryMvoMetadata.Instance.Properties)
            {
        ...
        [HttpGet]
        public InventoryItemRequirementEntryMvoStateCreatedOrMergePatchedOrDeletedDto GetStateEvent(string id, long version)
        {
          try {
            var idObj = InventoryItemRequirementEntryMvosControllerUtils.ParseIdString(id);
        ...
        [HttpGet]
        public IInventoryItemRequirementEntryMvoStateDto GetHistoryState(string id, long version, string fields = null)
        {
          try {
            var idObj = InventoryItemRequirementEntryMvosControllerUtils.ParseIdString(id);
            var state = _inventoryItemRequirementEntryMvoApplicationService.GetHistoryState(idObj, version);
            if (state == null) { return null; }
            var stateDto = new InventoryItemRequirementEntryMvoStateDtoWrapper(state);
            if (String.IsNullOrWhiteSpace(fields))
            {
        ...
        private class PropertyTypeResolver : IPropertyTypeResolver
        {
            ...
            {
                return InventoryItemRequirementEntryMvosControllerUtils.GetFilterPropertyType(propertyName);
            }
        }
    ...
    public static class InventoryItemRequirementEntryMvosControllerUtils
    {
        ...
        public static string GetFilterPropertyName(string fieldName)
        {
            ...
            if (InventoryItemRequirementEntryMvoMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(fieldName))
            {
                return InventoryItemRequirementEntryMvoMetadata.Instance.FilteringPropertyAliasDictionary[fieldName];
            }
            return null;
        }
        ...
        {
            if (InventoryItemRequirementEntryMvoMetadata.Instance.PropertyMetadataDictionary.ContainsKey(propertyName))
            {
                return InventoryItemRequirementEntryMvoMetadata.Instance.PropertyMetadataDictionary[propertyName].Type;
            }
        ...
        public static IEnumerable<IInventoryItemRequirementEntryMvoStateDto> ToInventoryItemRequirementEntryMvoStateDtoCollection(IEnumerable<InventoryItemRequirementEntryId> ids)
        {
            var states = new List<IInventoryItemRequirementEntryMvoStateDto>();
        ...
        public static IEnumerable<IInventoryItemRequirementEntryMvoState> ToInventoryItemRequirementEntryMvoStateCollection(IEnumerable<InventoryItemRequirementEntryId> ids)
        {
            var states = new List<InventoryItemRequirementEntryMvoState>();
            foreach (var id in ids)
            {
                var s = new InventoryItemRequirementEntryMvoState();
                s.InventoryItemRequirementEntryId = id;
                states.Add(s);
            }
            return states;
        }
