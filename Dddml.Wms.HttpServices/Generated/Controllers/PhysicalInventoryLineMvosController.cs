...
namespace Dddml.Wms.HttpServices.ApiControllers
{
    ...
    [RoutePrefix("api/PhysicalInventoryLineMvos")]
    public partial class PhysicalInventoryLineMvosController : ApiController
    {

        IPhysicalInventoryLineMvoApplicationService _physicalInventoryLineMvoApplicationService = ApplicationContext.Current["PhysicalInventoryLineMvoApplicationService"] as IPhysicalInventoryLineMvoApplicationService;
        ...
        public void Put(string id, [FromBody]CreatePhysicalInventoryLineMvoDto value)
        {
          try {
        ...
        [HttpGet]
        public IPhysicalInventoryLineMvoStateDto GetHistoryState(string id, long version, string fields = null)
        {
          try {
            var idObj = PhysicalInventoryLineMvosControllerUtils.ParseIdString(id);
            var state = _physicalInventoryLineMvoApplicationService.GetHistoryState(idObj, version);
            if (state == null) { return null; }
            var stateDto = new PhysicalInventoryLineMvoStateDtoWrapper(state);
            if (String.IsNullOrWhiteSpace(fields))
            {
    ...
    public static class PhysicalInventoryLineMvosControllerUtils
    {
        ...
        public static void SetNullIdOrThrowOnInconsistentIds(string id, CreateOrMergePatchOrDeletePhysicalInventoryLineMvoDto value)
        {
            var idObj = ParseIdString(id.IsNormalized() ? id : id.Normalize());
            if (value.PhysicalInventoryLineId == null)
            {
                value.PhysicalInventoryLineId = new PhysicalInventoryLineIdDtoWrapper(idObj);
            }
            else if (!((ICreateOrMergePatchOrDeletePhysicalInventoryLineMvo)value).PhysicalInventoryLineId.Equals(idObj))
            {
                throw DomainError.Named("inconsistentId", "Argument Id {0} NOT equals body Id {1}", id, value.PhysicalInventoryLineId);
            }
        }
        ...
        public static PhysicalInventoryLineId ParseIdString(string idString)
        {
            ...
            var rId = idDto.ToPhysicalInventoryLineId();
            return rId;
        }
        ...
        public static string GetFilterPropertyName(string fieldName)
        {
            ...
            if (PhysicalInventoryLineMvoMetadata.Instance.FilteringPropertyAliasDictionary.ContainsKey(fieldName))
            {
                return PhysicalInventoryLineMvoMetadata.Instance.FilteringPropertyAliasDictionary[fieldName];
            }
            return null;
        }
        ...
        {
            if (PhysicalInventoryLineMvoMetadata.Instance.PropertyMetadataDictionary.ContainsKey(propertyName))
            {
                return PhysicalInventoryLineMvoMetadata.Instance.PropertyMetadataDictionary[propertyName].Type;
            }
        ...
        public static IEnumerable<IPhysicalInventoryLineMvoStateDto> ToPhysicalInventoryLineMvoStateDtoCollection(IEnumerable<PhysicalInventoryLineId> ids)
        {
            var states = new List<IPhysicalInventoryLineMvoStateDto>();
            foreach (var id in ids)
            {
                var dto = new PhysicalInventoryLineMvoStateDtoWrapper();
                dto.PhysicalInventoryLineId = new PhysicalInventoryLineIdDtoWrapper(id);
                states.Add(dto);
            }
            return states;
        }
